...
class AdminService
{
    ...
    public function getList($arFilter = [], $arOrder = [], $arRelations = [],$customSort = false)
    {
        ...
        $arCustomFields = array_filter($this->model::$listFields, function ($arItem) {
            return isset($arItem['custom_fields']) && $arItem['custom_fields'] == 'Y';
        });
        if ($arOrder) {
            if (isset($arOrder['field'])) {
                $items = $this->model::orderBy($arOrder['field'], $arOrder['order']);
            } else {
                foreach ($arOrder as $order) {
                    $items = $this->model::orderBy($order['field'], $order['order']);
                }
            }
        } else {
        ...
        $date = time();
        $result = datatables()->of($items)->addColumn('check', function ($item) {
            return sprintf('<div class="center_column"><input type="checkbox" class="item-checker" name="items[]" value="%s"></div>',$item->id);
        })->setRowClass(function ($item) use ($date) {
            if (isset($item->date) && $item->date) {
                return strtotime($item->date) < $date ? 'dark-row' : '';
            } else {
                return '';
            }
        });
        ...
        return $result->rawColumns(array_merge($arLinkFields ? array_column($arLinkFields, 'data') : [], ['action','check'],
            $arCustomFields ? array_column($arCustomFields, 'data') : []))
            ->toJson();
    }
